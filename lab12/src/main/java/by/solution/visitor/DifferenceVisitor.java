...
public class DifferenceVisitor<K, V> implements Visitor<K, V> {

    private final MapModel<K, V> map;
    private final MapModel<K, V> differenceMap;

    public DifferenceVisitor(MapModel<K, V> map) {
        this.map = map;
        this.differenceMap = new MapModel<>();
    }
